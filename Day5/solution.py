""" Solves Day Five of  Advent of Code 2022 https://adventofcode.com/2022/day/5 """
# --- Day 5: Supply Stacks ---
...
def top_creates(crate_mover, stack_count=9) -> str:
    """reads initial create config and moves creates according to instructions,
    returns each top crate"""
    stacks = [[]]*stack_count  # [[], [], ...]
    ...
    with open("day5/input.txt", encoding="UTF-8") as file:
        ...
        for line in file.readlines():
            ...
            if not configuration_read:
                ...
                origin = int(line.split(" ")[3]) - 1
                target = int(line.split(" ")[5]) - 1
                # stack on new stack
                stacks[target] += crate_mover(stacks[origin], boxes)
                # remove from old stack
                stacks[origin] = stacks[origin][:-boxes]

    return "".join(str(s[-1]) for s in stacks)


def crate_mover_9000(stack, amount) -> list:
    """CrateMover 9000 can only grab one crate at a time"""
    return reversed(stack[-amount:])


print("top_creates with crate_mover_9000:", top_creates(crate_mover_9000))
...
def crate_mover_9001(stack, amount) -> list:
    """CrateMover 9001 can grab multiple crates at once"""
    return stack[-amount:]


print("top_creates with crate_mover_9001:", top_creates(crate_mover_9001))
