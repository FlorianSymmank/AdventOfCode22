""" Solves Day One of  Advent of Code 2022 https://adventofcode.com/2022/day/1 """
...
# rations, etc. that they've brought with them, one item per line. Each Elf separates their
...
def max_calories_carried() -> int:
    """ returns max calories carried """
    max_calories = 0
    with open("day1/input1.txt", encoding="UTF-8") as file:
        curr_calories = 0
        for line in file.readlines():
            line = line.replace("\n", "")
            if len(line) == 0:  # next elf
                max_calories = curr_calories if max_calories < curr_calories else max_calories
                curr_calories = 0
            else:
...
print("maxCalories: ", max_calories_carried())
...
def sum_nth_most_calories(nth: int = 3) -> int:
    """returns sum of nth most calories carried"""
    elves = []
    with open("day1/input1.txt", encoding="UTF-8") as file:
        calories = 0
        for line in file.readlines():
    ...
    return sum(sorted(elves, reverse=True)[0:nth])


print("summ_nth_MostCalories: ", sum_nth_most_calories())
