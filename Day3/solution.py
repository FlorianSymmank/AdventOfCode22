""" Solves Day Three of  Advent of Code 2022 https://adventofcode.com/2022/day/3 """
...
def get_priority(item: str) -> int:
    """lowercase items a = 1 to z= 26, uppercase A = 26 to Z = 52"""
    priority = 0
    if ord(item) < 91:  # is uppercase
        priority = 27 + ord(item) - 65
    else:
...
def sum_of_priorities() -> int:
    """sums up wrongly packed items priority, items are packed wrong
    if a item of same type is found in both compartments """
    ...
    with open("day3/input.txt", encoding="UTF-8") as file:
        for line in file.readlines():
            ...
            priority_sum += sum(get_priority(item)
                                for item in compartment1 if item in compartment2)
...
def sum_group_priorities(group_size: int = 3) -> int:
    """get sum of the shared item priority of all groups"""
    group_priority_sum = 0
    with open("day3/input.txt", encoding="UTF-8") as file:
        lines = file.readlines()
        for i in range(0, len(lines), group_size):  # step_size = group_size
            # reduce number of comparisons needed with set
            rucksacks = [set(x.replace("\n", ""))
                         for x in lines[i:i+group_size]]

            # since an item IS be shared, loop through rucksack[0] items
            for item in rucksacks[0]:
                ...
                if in_all:
                    group_priority_sum += get_priority(item)

    return group_priority_sum


print("sum_group_priorities: ", sum_group_priorities())
