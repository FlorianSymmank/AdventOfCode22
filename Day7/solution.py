...
class Directory:
    ...
    def get_subdir(self, name):
        """finds and returns the searched for subdir, also returns parentdir if '..' is seached """
        # HACK: also returns parentdir if '..' is seached
        if name == "..":
            return self.parent_directory

        for subddir in self.subdirectories:
            if subddir.name == name:
                return subddir
        return None

    def size(self):
        """returns size of containing files AND size of subdirs"""
        size = sum(f.size for f in self.files)
        size += sum(d.size() for d in self.subdirectories)
        return size

    def __str__(self):
        file_string = ", ".join(str(f) for f in self.files)
        subdir_string = ", ".join(d.name for d in self.subdirectories)

        return f"Directory: {self.name} Size: {self.size()}\nFiles: [{file_string}]\nSubdirectories: [{subdir_string}]"


def get_root() -> Directory:
    """returns the root of created filesystem"""
    ...
    with open("day7/input.txt", encoding="UTF-8") as file:
        for line in file.readlines():
            ...
            if root is None:
                root = Directory(parts[0], parts[0])
                current_dir = root
                continue

            if parts[0] == "$":
                if parts[1] == "cd":
                    current_dir = current_dir.get_subdir(parts[2])

            else:
                if parts[0] == "dir":
                    current_dir.add_subdir(Directory(current_dir, parts[1]))
                else:
                    current_dir.add_file(File(parts[1], int(parts[0])))

    return root


def total_size_of_dirs_with(max_dir_size: int) -> int:
    """Returns the total size of all directorys with size smaller than max_dir_size,
    files may be counted twice"""
    root = get_root()

    dirs = []
    dirs_to_check = [root]

    # traverses whole tree of dirs
    while len(dirs_to_check) > 0:
        directory = dirs_to_check.pop()
        dirs_to_check.extend(directory.subdirectories)
...
def optimal_dir_to_delete(filesystem_size: int, update_size: int) -> Directory:
    """get the optimal dir to delete, just big enough to be able to install the update"""
    root = get_root()
    ...
    while len(dirs_to_check) > 0:
        ...
        eligeble_dirs = \
            [d for d in current_dir.subdirectories if d.size() >= space_to_free]
        dirs_to_check.extend(eligeble_dirs)
        dirs.extend(eligeble_dirs)

    # sorted ascending size get smallest
    return sorted(dirs, key=lambda d: d.size())[0]


print("optimal_dir_to_delete:", str(
    optimal_dir_to_delete(70_000_000, 30_000_000)))
